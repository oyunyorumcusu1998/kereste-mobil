...
if len(st.session_state.veriler) > 0:
    ...
    # 2. ÖZET TABLO (GRUPLAMA)
    st.divider()
    st.subheader("📊 Cins Bazında Özet")
    
    # Pandas ile gruplama işlemi
    ozet_df = df.groupby("Ağaç Cinsi")["Hacim (m3)"].sum().reset_index()
    ozet_df.columns = ["Ağaç Cinsi", "Toplam Hacim (m3)"] # Başlıkları güzelleştir
    ...
    def create_pdf(dataframe, summary_df, total_m3):
        ...
        # --- TABLO 1: DETAYLAR ---
        elements.append(Paragraph("Detaylı Döküm:", styles['Heading4']))
        ...
        # Genel toplamı özetin altına ekle
        summary_data.append(["GENEL TOPLAM:", f"{total_m3:.4f}"])

        t_sum = Table(summary_data, colWidths=[150, 150])
        style_sum = TableStyle([
            ('FONTNAME', (0, 0), (-1, -1), tr_font),
            ('BACKGROUND', (0, 0), (-1, 0), colors.firebrick), # Özet tablosu kırmızı başlıklı olsun
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ('FONTNAME', (0, 0), (-1, 0), tr_font),
            ('FONTNAME', (0, -1), (-1, -1), tr_font), # Toplam kalın
            ('GRID', (0, 0), (-1, -1), 1, colors.black),
            ('BACKGROUND', (0, -1), (-1, -1), colors.lightgrey), # En alt satır gri
        ])
